...
***************************************/
    ...
    static const uint8 CYCODE DEBUGPRINT_MIDI_SIZE[] = {
...
***************************************/
        ...
        * USBMIDI_MIDI_EP_Init() function.*/
        volatile uint16 DEBUGPRINT_midiInPointer;                            /* Input endpoint buffer pointer */
    #else
        volatile uint8 DEBUGPRINT_midiInPointer;                             /* Input endpoint buffer pointer */
    ...
    volatile uint8 DEBUGPRINT_midi_in_ep;
    ...
    uint8 DEBUGPRINT_midiInBuffer[DEBUGPRINT_MIDI_IN_BUFF_SIZE];       /* Input endpoint buffer */
    ...
    volatile uint8 DEBUGPRINT_midi_out_ep;                                   /* Output endpoint number */
    ...
    uint8 DEBUGPRINT_midiOutBuffer[DEBUGPRINT_MIDI_OUT_BUFF_SIZE];     /* Output endpoint buffer */
#endif /* (DEBUGPRINT_MIDI_OUT_BUFF_SIZE > 0) */

#if (DEBUGPRINT_MIDI_EXT_MODE >= DEBUGPRINT_ONE_EXT_INTRF)

    static DEBUGPRINT_MIDI_RX_STATUS DEBUGPRINT_MIDI1_Event;            /* MIDI RX status structure */
    static volatile uint8 DEBUGPRINT_MIDI1_TxRunStat;                         /* MIDI Output running status */
    ...
    volatile uint8 DEBUGPRINT_MIDI1_InqFlags;                                 /* Device inquiry flag */

    #if (DEBUGPRINT_MIDI_EXT_MODE >= DEBUGPRINT_TWO_EXT_INTRF)
        static DEBUGPRINT_MIDI_RX_STATUS DEBUGPRINT_MIDI2_Event;        /* MIDI RX status structure */
        static volatile uint8 DEBUGPRINT_MIDI2_TxRunStat;                     /* MIDI Output running status */
        /** See description of \ref DEBUGPRINT_MIDI1_InqFlags*/
        volatile uint8 DEBUGPRINT_MIDI2_InqFlags;                             /* Device inquiry flag */
...
#if (DEBUGPRINT_ENABLE_MIDI_API != 0u)
/*******************************************************************************
* Function Name: DEBUGPRINT_MIDI_Init
****************************************************************************//**
*
...
* \globalvars
*
*  \ref DEBUGPRINT_midiInBuffer: This buffer is used for saving and combining
*    the received data from UART(s) and(or) generated internally by
*    PutUsbMidiIn() function messages. DEBUGPRINT_MIDI_IN_EP_Service()
*    function transfers the data from this buffer to the PC.
*
*  \ref DEBUGPRINT_midiOutBuffer: This buffer is used by the
*    DEBUGPRINT_MIDI_OUT_Service() function for saving the received
*    from the PC data, then the data are parsed and transferred to UART(s)
*    buffer and to the internal processing by the
*
*  \ref DEBUGPRINT_callbackLocalMidiEvent function.
*
*  \ref DEBUGPRINT_midi_out_ep: Used as an OUT endpoint number.
*
*  \ref DEBUGPRINT_midi_in_ep: Used as an IN endpoint number.
*
*   \ref DEBUGPRINT_midiInPointer: Initialized to zero.
*
* \sideeffect
...
void DEBUGPRINT_MIDI_Init(void) 
{
   ...
   DEBUGPRINT_midiInPointer = 0u;
        ...
        DEBUGPRINT_LoadInEP(DEBUGPRINT_midi_in_ep, DEBUGPRINT_midiInBuffer,
                                                               DEBUGPRINT_MIDI_IN_BUFF_SIZE);
    #endif  /* (DEBUGPRINT_MIDI_IN_BUFF_SIZE > 0) */

    #if (DEBUGPRINT_MIDI_OUT_BUFF_SIZE > 0)
        /* Provide buffer for OUT endpoint. */
        (void)DEBUGPRINT_ReadOutEP(DEBUGPRINT_midi_out_ep, DEBUGPRINT_midiOutBuffer,
                                                                       DEBUGPRINT_MIDI_OUT_BUFF_SIZE);
    #endif /* (DEBUGPRINT_MIDI_OUT_BUFF_SIZE > 0) */
#endif /* (DEBUGPRINT_EP_MANAGEMENT_DMA_AUTO) */

#if (DEBUGPRINT_MIDI_OUT_BUFF_SIZE > 0)
    DEBUGPRINT_EnableOutEP(DEBUGPRINT_midi_out_ep);
#endif /* (DEBUGPRINT_MIDI_OUT_BUFF_SIZE > 0) */

    /* Initialize the MIDI port(s) */
#if (DEBUGPRINT_MIDI_EXT_MODE >= DEBUGPRINT_ONE_EXT_INTRF)
    DEBUGPRINT_MIDI_InitInterface();
#endif /* (DEBUGPRINT_MIDI_EXT_MODE >= DEBUGPRINT_ONE_EXT_INTRF) */
}
...
#if (DEBUGPRINT_MIDI_OUT_BUFF_SIZE > 0)
    /*******************************************************************************
    * Function Name: DEBUGPRINT_MIDI_OUT_Service
    ****************************************************************************//**
    *
    ...
    * \globalvars
    *
    *  \ref DEBUGPRINT_midiOutBuffer: Used as temporary buffer between USB
    ...
    *******************************************************************************/
    ...
    {
    #if (DEBUGPRINT_MIDI_OUT_BUFF_SIZE >= 256)
        uint16 outLength;
        uint16 outPointer;
    #else
        uint8 outLength;
        uint8 outPointer;
    #endif /* (DEBUGPRINT_MIDI_OUT_BUFF_SIZE >= 256) */

        /* Service the USB MIDI output endpoint. */
        if (DEBUGPRINT_OUT_BUFFER_FULL == DEBUGPRINT_GetEPState(DEBUGPRINT_midi_out_ep))
        {
        #if (DEBUGPRINT_MIDI_OUT_BUFF_SIZE >= 256)
            outLength = DEBUGPRINT_GetEPCount(DEBUGPRINT_midi_out_ep);
        #else
            outLength = (uint8)DEBUGPRINT_GetEPCount(DEBUGPRINT_midi_out_ep);
        #endif /* (DEBUGPRINT_MIDI_OUT_BUFF_SIZE >= 256) */

        #if (!DEBUGPRINT_EP_MANAGEMENT_DMA_AUTO)
            #if (DEBUGPRINT_MIDI_OUT_BUFF_SIZE >= 256)
                outLength = DEBUGPRINT_ReadOutEP(DEBUGPRINT_midi_out_ep,
                                                       DEBUGPRINT_midiOutBuffer, outLength);
            #else
                outLength = (uint8)DEBUGPRINT_ReadOutEP(DEBUGPRINT_midi_out_ep,
                                                              DEBUGPRINT_midiOutBuffer, (uint16) outLength);
            #endif /* (DEBUGPRINT_MIDI_OUT_BUFF_SIZE >= 256) */

            #if (DEBUGPRINT_EP_MANAGEMENT_DMA_MANUAL)
                /* Wait until DMA complete transferring data from OUT endpoint buffer. */
                while (DEBUGPRINT_OUT_BUFFER_FULL == DEBUGPRINT_GetEPState(DEBUGPRINT_midi_out_ep))
                {
                }
            ...
            if (outLength >= DEBUGPRINT_EVENT_LENGTH)
            {
                ...
                #if (DEBUGPRINT_MIDI_EXT_MODE >= DEBUGPRINT_ONE_EXT_INTRF)
                    if ((DEBUGPRINT_midiOutBuffer[outPointer] & DEBUGPRINT_CABLE_MASK) ==
                         DEBUGPRINT_MIDI_CABLE_00)
                    {
                        DEBUGPRINT_MIDI1_ProcessUsbOut(&DEBUGPRINT_midiOutBuffer[outPointer]);
                    }
                    else if ((DEBUGPRINT_midiOutBuffer[outPointer] & DEBUGPRINT_CABLE_MASK) ==
                             DEBUGPRINT_MIDI_CABLE_01)
                    {
                    #if (DEBUGPRINT_MIDI_EXT_MODE >= DEBUGPRINT_TWO_EXT_INTRF)
                         DEBUGPRINT_MIDI2_ProcessUsbOut(&DEBUGPRINT_midiOutBuffer[outPointer]);
                    #endif /*  DEBUGPRINT_MIDI_EXT_MODE >= DEBUGPRINT_TWO_EXT_INTRF */
                    }
                    else
                    {
                        ...
                        #ifdef DEBUGPRINT_MIDI_OUT_EP_SERVICE_CALLBACK
                            DEBUGPRINT_MIDI_OUT_EP_Service_Callback();
                        #endif /* DEBUGPRINT_MIDI_OUT_EP_SERVICE_CALLBACK */
                    }
                #endif /* (DEBUGPRINT_MIDI_EXT_MODE >= DEBUGPRINT_ONE_EXT_INTRF) */

                    /* Process any local MIDI output functions */
                    DEBUGPRINT_callbackLocalMidiEvent(DEBUGPRINT_midiOutBuffer[outPointer] & DEBUGPRINT_CABLE_MASK,
                                                            &DEBUGPRINT_midiOutBuffer[outPointer + DEBUGPRINT_EVENT_BYTE1]);
                    outPointer += DEBUGPRINT_EVENT_LENGTH;
                }
            }
...
#if (DEBUGPRINT_MIDI_IN_BUFF_SIZE > 0)
    /*******************************************************************************
    * Function Name: DEBUGPRINT_MIDI_IN_EP_Service
    ****************************************************************************//**
    *
    ...
    *  DEBUGPRINT_MIDI_IN_Service() and from the
    *  DEBUGPRINT_PutUsbMidiIn() function.
    *
    * \globalvars
    *  DEBUGPRINT_midi_in_ep: Used as an IN endpoint number.
    *  DEBUGPRINT_midiInBuffer: Function loads the data from this buffer to
    *    the USB IN endpoint.
    *   DEBUGPRINT_midiInPointer: Cleared to zero when data are sent.
    *
    * \reentrant
    *  No
    *
    *******************************************************************************/
    void DEBUGPRINT_MIDI_IN_EP_Service(void) 
    {
        ...
        if (DEBUGPRINT_midiInPointer != 0u)
        {
            if(DEBUGPRINT_GetEPState(DEBUGPRINT_midi_in_ep) == DEBUGPRINT_EVENT_PENDING)
            {
            #if (DEBUGPRINT_EP_MANAGEMENT_DMA_AUTO)
                DEBUGPRINT_LoadInEP(DEBUGPRINT_midi_in_ep, NULL, (uint16)DEBUGPRINT_midiInPointer);
            #else
                DEBUGPRINT_LoadInEP(DEBUGPRINT_midi_in_ep, DEBUGPRINT_midiInBuffer,
                                                              (uint16) DEBUGPRINT_midiInPointer);
            #endif /* (DEBUGPRINT_EP_MANAGEMENT_DMA_AUTO) */
    ...
    *******************************************************************************/
    void DEBUGPRINT_MIDI_IN_Service(void) 
    {
        ...
        */
        ...
        if (0u == DEBUGPRINT_midiInPointer)
        {
            do
            {
                if (DEBUGPRINT_midiInPointer <= (DEBUGPRINT_MIDI_IN_BUFF_SIZE - DEBUGPRINT_EVENT_LENGTH))
                {
                    /* Check MIDI1 input port for a complete event */
                    m1 = DEBUGPRINT_MIDI1_GetEvent();
                    if (m1 != 0u)
                    {
                        DEBUGPRINT_PrepareInBuffer(m1, (uint8 *)&DEBUGPRINT_MIDI1_Event.msgBuff[0],
                                                                       DEBUGPRINT_MIDI1_Event.size, DEBUGPRINT_MIDI_CABLE_00);
                    }
                }
                ...
                if (DEBUGPRINT_midiInPointer <= (DEBUGPRINT_MIDI_IN_BUFF_SIZE - DEBUGPRINT_EVENT_LENGTH))
                {
                    ...
                    m2 = DEBUGPRINT_MIDI2_GetEvent();
                    if (m2 != 0u)
                    {
                        DEBUGPRINT_PrepareInBuffer(m2, (uint8 *)&DEBUGPRINT_MIDI2_Event.msgBuff[0],
                                                                       DEBUGPRINT_MIDI2_Event.size, DEBUGPRINT_MIDI_CABLE_01);
                    }
                }
            #endif /*  DEBUGPRINT_MIDI_EXT_MODE >= DEBUGPRINT_TWO_EXT_INTRF */
            }
            while((DEBUGPRINT_midiInPointer <= (DEBUGPRINT_MIDI_IN_BUFF_SIZE - DEBUGPRINT_EVENT_LENGTH)) &&
                  ((m1 != 0u) || (m2 != 0u)));
        }
    #endif /* (DEBUGPRINT_MIDI_EXT_MODE >= DEBUGPRINT_ONE_EXT_INTRF) */

        /* Service the USB MIDI input endpoint */
        DEBUGPRINT_MIDI_IN_EP_Service();
    }


    /***************************************************************************
    * Function Name: DEBUGPRINT_PutUsbMidiIn
    ************************************************************************//**
    *
    ...
    *  The DEBUGPRINT_MIDI_IN_Service() function should also be called to
    ...
    *   ----------------------|-----------------------------------------
    *  DEBUGPRINT_TRUE  | Host is not ready to receive this message
    *  DEBUGPRINT_FALSE | Success transfer
    *
    * \globalvars
    *
    *  \ref DEBUGPRINT_midi_in_ep: MIDI IN endpoint number used for
    *        sending data.
    *
    *  \ref DEBUGPRINT_midiInPointer: Checked this variable to see if
    ...
    ***************************************************************************/
    ...
    {
        uint8 retError = DEBUGPRINT_FALSE;
        uint8 msgIndex;
        ...
        if (DEBUGPRINT_midiInPointer >
                    (DEBUGPRINT_EP[DEBUGPRINT_midi_in_ep].bufferSize - DEBUGPRINT_EVENT_LENGTH))
        {
            DEBUGPRINT_MIDI_IN_EP_Service();
        }
        ...
        if (DEBUGPRINT_midiInPointer <=
                    (DEBUGPRINT_EP[DEBUGPRINT_midi_in_ep].bufferSize - DEBUGPRINT_EVENT_LENGTH))
        {
            if((ic < DEBUGPRINT_EVENT_LENGTH) || (ic >= DEBUGPRINT_MIDI_STATUS_MASK))
            {
                DEBUGPRINT_PrepareInBuffer(ic, midiMsg, ic, cable);
            }
            else
            {
                ...
                do
                {
                    DEBUGPRINT_PrepareInBuffer(DEBUGPRINT_MIDI_SYSEX, &midiMsg[msgIndex],
                                                     DEBUGPRINT_EVENT_BYTE3, cable);

                    ic -= DEBUGPRINT_EVENT_BYTE3;
                    msgIndex += DEBUGPRINT_EVENT_BYTE3;
                        ...
                        (DEBUGPRINT_EP[DEBUGPRINT_midi_in_ep].bufferSize - DEBUGPRINT_EVENT_LENGTH))
                    {
                           ...
                           (DEBUGPRINT_EP[DEBUGPRINT_midi_in_ep].bufferSize - DEBUGPRINT_EVENT_LENGTH))
                        {
                            ...
                            retError = DEBUGPRINT_TRUE;
                            break;
                        }
                    }
                }
                while (ic > DEBUGPRINT_EVENT_BYTE3);

                if (retError == DEBUGPRINT_FALSE)
                {
                    DEBUGPRINT_PrepareInBuffer(DEBUGPRINT_MIDI_EOSEX, midiMsg, ic, cable);
                }
            }
        }
        else
    ...
    /*******************************************************************************
    * Function Name: DEBUGPRINT_PrepareInBuffer
    ****************************************************************************//**
    *
    ...
    *
    * \globalvars
    *  DEBUGPRINT_midiInBuffer: This buffer is used for saving and combine the
    *    received from UART(s) and(or) generated internally by
    *    DEBUGPRINT_PutUsbMidiIn() function messages.
    *  DEBUGPRINT_midiInPointer: Used as an index for midiInBuffer to
    *     write data.
    *
    * \reentrant
    *  No
    ...
    *******************************************************************************/
                                                                 ...
                                                                 
    {
        ...
        if (ic >= (DEBUGPRINT_MIDI_STATUS_MASK | DEBUGPRINT_MIDI_SINGLE_BYTE_MASK))
        {
            DEBUGPRINT_midiInBuffer[DEBUGPRINT_midiInPointer] = DEBUGPRINT_SINGLE_BYTE | cable;
            DEBUGPRINT_midiInPointer++;
            DEBUGPRINT_midiInBuffer[DEBUGPRINT_midiInPointer] = ic;
            DEBUGPRINT_midiInPointer++;
            DEBUGPRINT_midiInBuffer[DEBUGPRINT_midiInPointer] = 0u;
            DEBUGPRINT_midiInPointer++;
            DEBUGPRINT_midiInBuffer[DEBUGPRINT_midiInPointer] = 0u;
            DEBUGPRINT_midiInPointer++;
        }
        else if((ic < DEBUGPRINT_EVENT_LENGTH) || (ic == DEBUGPRINT_MIDI_SYSEX))
        {
            if(ic == DEBUGPRINT_MIDI_SYSEX)
            {
                DEBUGPRINT_midiInBuffer[DEBUGPRINT_midiInPointer] = DEBUGPRINT_SYSEX | cable;
                DEBUGPRINT_midiInPointer++;
            }
            else if (srcBuffZero < DEBUGPRINT_MIDI_SYSEX)
            {
                DEBUGPRINT_midiInBuffer[DEBUGPRINT_midiInPointer] = (srcBuffZero >> 4u) | cable;
                DEBUGPRINT_midiInPointer++;
            }
            else if (srcBuffZero == DEBUGPRINT_MIDI_TUNEREQ)
            {
                DEBUGPRINT_midiInBuffer[DEBUGPRINT_midiInPointer] = DEBUGPRINT_1BYTE_COMMON | cable;
                DEBUGPRINT_midiInPointer++;
            }
            else if ((srcBuffZero == DEBUGPRINT_MIDI_QFM) || (srcBuffZero == DEBUGPRINT_MIDI_SONGSEL))
            {
                DEBUGPRINT_midiInBuffer[DEBUGPRINT_midiInPointer] = DEBUGPRINT_2BYTE_COMMON | cable;
                DEBUGPRINT_midiInPointer++;
            }
            else if (srcBuffZero == DEBUGPRINT_MIDI_SPP)
            {
                DEBUGPRINT_midiInBuffer[DEBUGPRINT_midiInPointer] = DEBUGPRINT_3BYTE_COMMON | cable;
                DEBUGPRINT_midiInPointer++;
            }
            else
            {
            ...
            DEBUGPRINT_midiInBuffer[DEBUGPRINT_midiInPointer] = srcBuffZero;
            DEBUGPRINT_midiInPointer++;
            DEBUGPRINT_midiInBuffer[DEBUGPRINT_midiInPointer] = srcBuffOne;
            DEBUGPRINT_midiInPointer++;
            DEBUGPRINT_midiInBuffer[DEBUGPRINT_midiInPointer] = srcBuff[2u];
            DEBUGPRINT_midiInPointer++;
        }
        else if (ic == DEBUGPRINT_MIDI_EOSEX)
        {
            switch (eventLen)
            {
                case 0u:
                    DEBUGPRINT_midiInBuffer[DEBUGPRINT_midiInPointer] =
                                                                        DEBUGPRINT_SYSEX_ENDS_WITH1 | cable;
                    DEBUGPRINT_midiInPointer++;
                    DEBUGPRINT_midiInBuffer[DEBUGPRINT_midiInPointer] = DEBUGPRINT_MIDI_EOSEX;
                    DEBUGPRINT_midiInPointer++;
                    DEBUGPRINT_midiInBuffer[DEBUGPRINT_midiInPointer] = 0u;
                    DEBUGPRINT_midiInPointer++;
                    DEBUGPRINT_midiInBuffer[DEBUGPRINT_midiInPointer] = 0u;
                ...
                case 1u:
                    DEBUGPRINT_midiInBuffer[DEBUGPRINT_midiInPointer] =
                                                                        DEBUGPRINT_SYSEX_ENDS_WITH2 | cable;
                    DEBUGPRINT_midiInPointer++;
                    DEBUGPRINT_midiInBuffer[DEBUGPRINT_midiInPointer] = srcBuffZero;
                    DEBUGPRINT_midiInPointer++;
                    DEBUGPRINT_midiInBuffer[DEBUGPRINT_midiInPointer] = DEBUGPRINT_MIDI_EOSEX;
                ...
                case 2u:
                    DEBUGPRINT_midiInBuffer[DEBUGPRINT_midiInPointer] =
                                                                        DEBUGPRINT_SYSEX_ENDS_WITH3 | cable;
                    DEBUGPRINT_midiInPointer++;
                    DEBUGPRINT_midiInBuffer[DEBUGPRINT_midiInPointer] = srcBuffZero;
                    DEBUGPRINT_midiInPointer++;
                    DEBUGPRINT_midiInBuffer[DEBUGPRINT_midiInPointer] = srcBuffOne;
...
*/
#if (DEBUGPRINT_MIDI_EXT_MODE >= DEBUGPRINT_ONE_EXT_INTRF)
    /*******************************************************************************
    * Function Name: DEBUGPRINT_MIDI_InitInterface
    ****************************************************************************//**
    *
    ...
    *
    * \globalvars
    *   DEBUGPRINT_MIDI_Event: initialized to zero.
    *   DEBUGPRINT_MIDI_TxRunStat: initialized to zero.
    *
    *******************************************************************************/
    void DEBUGPRINT_MIDI_InitInterface(void) 
    {
        DEBUGPRINT_MIDI1_Event.length  = 0u;
        DEBUGPRINT_MIDI1_Event.count   = 0u;
        DEBUGPRINT_MIDI1_Event.size    = 0u;
        DEBUGPRINT_MIDI1_Event.runstat = 0u;
        DEBUGPRINT_MIDI1_TxRunStat     = 0u;
        DEBUGPRINT_MIDI1_InqFlags      = 0u;

        /* Start UART block */
        MIDI1_UART_Start();

        /* Change the priority of the UART TX and RX interrupt */
        CyIntSetPriority(MIDI1_UART_TX_VECT_NUM, DEBUGPRINT_CUSTOM_UART_TX_PRIOR_NUM);
        CyIntSetPriority(MIDI1_UART_RX_VECT_NUM, DEBUGPRINT_CUSTOM_UART_RX_PRIOR_NUM);

    #if (DEBUGPRINT_MIDI_EXT_MODE >= DEBUGPRINT_TWO_EXT_INTRF)
        DEBUGPRINT_MIDI2_Event.length  = 0u;
        DEBUGPRINT_MIDI2_Event.count   = 0u;
        DEBUGPRINT_MIDI2_Event.size    = 0u;
        DEBUGPRINT_MIDI2_Event.runstat = 0u;
        DEBUGPRINT_MIDI2_TxRunStat     = 0u;
        DEBUGPRINT_MIDI2_InqFlags      = 0u;

        /* Start second UART block */
        MIDI2_UART_Start();

        /* Change the priority of the UART TX interrupt */
        CyIntSetPriority(MIDI2_UART_TX_VECT_NUM, DEBUGPRINT_CUSTOM_UART_TX_PRIOR_NUM);
        CyIntSetPriority(MIDI2_UART_RX_VECT_NUM, DEBUGPRINT_CUSTOM_UART_RX_PRIOR_NUM);
    #endif /* (DEBUGPRINT_MIDI_EXT_MODE >= DEBUGPRINT_TWO_EXT_INTRF) */
    ...
    #ifdef DEBUGPRINT_MIDI_INIT_CALLBACK
        DEBUGPRINT_MIDI_Init_Callback();
    #endif /* (DEBUGPRINT_MIDI_INIT_CALLBACK) */
    }


    /*******************************************************************************
    * Function Name: DEBUGPRINT_ProcessMidiIn
    ****************************************************************************//**
    *
    ...
    *******************************************************************************/
                                                                ...
                                                                
    {
        ...
        */
        if ((mData & DEBUGPRINT_MIDI_STATUS_BYTE_MASK) != 0u)
        {
            if ((mData & DEBUGPRINT_MIDI_STATUS_MASK) == DEBUGPRINT_MIDI_STATUS_MASK)
            {
                if ((mData & DEBUGPRINT_MIDI_SINGLE_BYTE_MASK) != 0u) /* System Real-Time Messages(single byte) */
                {
                    midiReturn = mData;
                }
                else                              /* System Common Messages */
                {
                    switch (mData)
                    {
                        case DEBUGPRINT_MIDI_SYSEX:
                            rxStat->msgBuff[0u] = DEBUGPRINT_MIDI_SYSEX;
                            rxStat->runstat = DEBUGPRINT_MIDI_SYSEX;
                            rxStat->count = 1u;
                            rxStat->length = 3u;
                            break;
                        case DEBUGPRINT_MIDI_EOSEX:
                            rxStat->runstat = 0u;
                            rxStat->size = rxStat->count;
                            rxStat->count = 0u;
                            midiReturn = DEBUGPRINT_MIDI_EOSEX;
                            break;
                        case DEBUGPRINT_MIDI_SPP:
                            rxStat->msgBuff[0u] = DEBUGPRINT_MIDI_SPP;
                            rxStat->runstat = 0u;
                            rxStat->count = 1u;
                            rxStat->length = 3u;
                            break;
                        case DEBUGPRINT_MIDI_SONGSEL:
                            rxStat->msgBuff[0u] = DEBUGPRINT_MIDI_SONGSEL;
                            rxStat->runstat = 0u;
                            rxStat->count = 1u;
                            rxStat->length = 2u;
                            break;
                        case DEBUGPRINT_MIDI_QFM:
                            rxStat->msgBuff[0u] = DEBUGPRINT_MIDI_QFM;
                            rxStat->runstat = 0u;
                            rxStat->count = 1u;
                            rxStat->length = 2u;
                            break;
                        case DEBUGPRINT_MIDI_TUNEREQ:
                            rxStat->msgBuff[0u] = DEBUGPRINT_MIDI_TUNEREQ;
                            rxStat->runstat = 0u;
                            rxStat->size = 1u;
                            rxStat->count = 0u;
            ...
            else /* Channel Messages */
            {
                ...
                rxStat->count = 1u;
                switch (mData & DEBUGPRINT_MIDI_STATUS_MASK)
                {
                    case DEBUGPRINT_MIDI_NOTE_OFF:
                    case DEBUGPRINT_MIDI_NOTE_ON:
                    case DEBUGPRINT_MIDI_POLY_KEY_PRESSURE:
                    case DEBUGPRINT_MIDI_CONTROL_CHANGE:
                    case DEBUGPRINT_MIDI_PITCH_BEND_CHANGE:
                        rxStat->length = 3u;
                        break;
                    case DEBUGPRINT_MIDI_PROGRAM_CHANGE:
                    case DEBUGPRINT_MIDI_CHANNEL_PRESSURE:
                        rxStat->length = 2u;
                        break;
                    default:
                        rxStat->runstat = 0u;
        ...
        {
            if (rxStat->runstat == DEBUGPRINT_MIDI_SYSEX)
            {
                ...
                if (rxStat->count >= rxStat->length)
                {
                    rxStat->size = rxStat->count;
                    rxStat->count = 0u;
                    midiReturn = DEBUGPRINT_MIDI_SYSEX;
                }
            }
            ...
            else if (rxStat->runstat != 0u)
            {
                ...
                rxStat->count = 2u;
                switch (rxStat->runstat & DEBUGPRINT_MIDI_STATUS_MASK)
                {
                    case DEBUGPRINT_MIDI_NOTE_OFF:
                    case DEBUGPRINT_MIDI_NOTE_ON:
                    case DEBUGPRINT_MIDI_POLY_KEY_PRESSURE:
    ...
    /*******************************************************************************
    * Function Name: DEBUGPRINT_MIDI1_GetEvent
    ****************************************************************************//**
    *
    ...
    *
    * \globalvars
    *  DEBUGPRINT_MIDI1_Event: RX status structure used to parse received
    *    data.
    *
    *******************************************************************************/
            ...
            */
            ...
            #endif /*  ((MIDI1_UART_RXBUFFERSIZE >= 256u) && (CY_PSOC3)) */
                {
                    ...
                    */
                    ...
                    msgRtn = DEBUGPRINT_ProcessMidiIn(rxData,
                                                    (DEBUGPRINT_MIDI_RX_STATUS *)&DEBUGPRINT_MIDI1_Event);
    ...
    /*******************************************************************************
    * Function Name: DEBUGPRINT_MIDI1_ProcessUsbOut
    ****************************************************************************//**
    *
    ...
    *
    * \globalvars
    *  DEBUGPRINT_MIDI1_TxRunStat: This variable used to save the MIDI
    *    status byte and skip to send the repeated status byte in subsequent event.
    *  DEBUGPRINT_MIDI1_InqFlags: The following flags are set when SysEx
    *    message comes.
    *    DEBUGPRINT_INQ_SYSEX_FLAG: Non-Real Time SySEx message received.
    *    DEBUGPRINT_INQ_IDENTITY_REQ_FLAG: Identity Request received.
    ...
    *******************************************************************************/
    ...
    #ifdef DEBUGPRINT_MIDI1_PROCESS_USB_OUT_ENTRY_CALLBACK
        DEBUGPRINT_MIDI1_ProcessUsbOut_EntryCallback();
    #endif /* (DEBUGPRINT_MIDI1_PROCESS_USB_OUT_ENTRY_CALLBACK) */

        cmd = epBuf[DEBUGPRINT_EVENT_BYTE0] & DEBUGPRINT_CIN_MASK;

        if ((cmd != DEBUGPRINT_RESERVED0) && (cmd != DEBUGPRINT_RESERVED1))
        {
            len = DEBUGPRINT_MIDI_SIZE[cmd];
            i = DEBUGPRINT_EVENT_BYTE1;
            /* Universal System Exclusive message parsing */
            if (cmd == DEBUGPRINT_SYSEX)
            {
                if ((epBuf[DEBUGPRINT_EVENT_BYTE1] == DEBUGPRINT_MIDI_SYSEX) &&
                    (epBuf[DEBUGPRINT_EVENT_BYTE2] == DEBUGPRINT_MIDI_SYSEX_NON_REAL_TIME))
                {
                    /* Non-Real Time SySEx starts */
                    DEBUGPRINT_MIDI1_InqFlags |= DEBUGPRINT_INQ_SYSEX_FLAG;
                }
                else
                {
                    DEBUGPRINT_MIDI1_InqFlags &= (uint8)~DEBUGPRINT_INQ_SYSEX_FLAG;
                }
            }
            else if (cmd == DEBUGPRINT_SYSEX_ENDS_WITH1)
            {
                DEBUGPRINT_MIDI1_InqFlags &= (uint8)~DEBUGPRINT_INQ_SYSEX_FLAG;
            }
            else if (cmd == DEBUGPRINT_SYSEX_ENDS_WITH2)
            {
                DEBUGPRINT_MIDI1_InqFlags &= (uint8)~DEBUGPRINT_INQ_SYSEX_FLAG;
            }
            else if (cmd == DEBUGPRINT_SYSEX_ENDS_WITH3)
            {
                /* Identify Request support */
                if ((DEBUGPRINT_MIDI1_InqFlags & DEBUGPRINT_INQ_SYSEX_FLAG) != 0u)
                {
                    DEBUGPRINT_MIDI1_InqFlags &= (uint8)~DEBUGPRINT_INQ_SYSEX_FLAG;
                    if ((epBuf[DEBUGPRINT_EVENT_BYTE1] == DEBUGPRINT_MIDI_SYSEX_GEN_INFORMATION) &&
                        (epBuf[DEBUGPRINT_EVENT_BYTE2] == DEBUGPRINT_MIDI_SYSEX_IDENTITY_REQ))
                    {
                        /* Set the flag about received the Identity Request.
                        *  The Identity Reply message may be send by user code.
                        */
                        DEBUGPRINT_MIDI1_InqFlags |= DEBUGPRINT_INQ_IDENTITY_REQ_FLAG;
                    }
                }
            }
            ...
            if ((cmd >= DEBUGPRINT_NOTE_OFF) && (cmd <= DEBUGPRINT_PITCH_BEND_CHANGE))
            {
                if (DEBUGPRINT_MIDI1_TxRunStat == epBuf[DEBUGPRINT_EVENT_BYTE1])
                {
                ...
                else
                {
                    ...
                    DEBUGPRINT_MIDI1_TxRunStat = epBuf[DEBUGPRINT_EVENT_BYTE1];
                }
            }
            else
            {
    ...
    #ifdef DEBUGPRINT_MIDI1_PROCESS_USB_OUT_EXIT_CALLBACK
        DEBUGPRINT_MIDI1_ProcessUsbOut_ExitCallback();
    #endif /* (DEBUGPRINT_MIDI1_PROCESS_USB_OUT_EXIT_CALLBACK) */
    }


#if (DEBUGPRINT_MIDI_EXT_MODE >= DEBUGPRINT_TWO_EXT_INTRF)
    /*******************************************************************************
    * Function Name: DEBUGPRINT_MIDI2_GetEvent
    ****************************************************************************//**
    *
    *  Checks for incoming MIDI data, calls the MIDI event builder if so.
    *  Returns either empty or with a complete event.
    ...
    *
    * \globalvars
    *  DEBUGPRINT_MIDI2_Event: RX status structure used to parse received
    *    data.
    *
    *******************************************************************************/
            ...
            */
            ...
            #endif /* ((MIDI2_UART_RXBUFFERSIZE >= 256u) && (CY_PSOC3)) */
                {
                    ...
                    */
                    ...
                    msgRtn = DEBUGPRINT_ProcessMidiIn(rxData,
                                                    (DEBUGPRINT_MIDI_RX_STATUS *)&DEBUGPRINT_MIDI2_Event);
    ...
    /*******************************************************************************
    * Function Name: DEBUGPRINT_MIDI2_ProcessUsbOut
    ****************************************************************************//**
    *
    *  Process a USB MIDI output event.
    *  Puts data into the MIDI TX output buffer.
    ...
    *
    * \globalvars
    *  DEBUGPRINT_MIDI2_TxRunStat: This variable used to save the MIDI
    *    status byte and skip to send the repeated status byte in subsequent event.
    *  DEBUGPRINT_MIDI2_InqFlags: The following flags are set when SysEx
    *    message comes.
    *  DEBUGPRINT_INQ_SYSEX_FLAG: Non-Real Time SySEx message received.
    *  DEBUGPRINT_INQ_IDENTITY_REQ_FLAG: Identity Request received.
    *    This bit should be cleared by user when Identity Reply message generated.
    ...
    *******************************************************************************/
                                                            ...
                                                            
    {
    ...
    #ifdef DEBUGPRINT_MIDI2_PROCESS_USB_OUT_ENTRY_CALLBACK
        DEBUGPRINT_MIDI2_ProcessUsbOut_EntryCallback();
    #endif /* (DEBUGPRINT_MIDI2_PROCESS_USB_OUT_ENTRY_CALLBACK) */
        ...
        if ((cmd != DEBUGPRINT_RESERVED0) && (cmd != DEBUGPRINT_RESERVED1))
        {
            ...
            if(cmd == DEBUGPRINT_SYSEX)
            {
                ...
                {
                    /* SySEx starts */
                    DEBUGPRINT_MIDI2_InqFlags |= DEBUGPRINT_INQ_SYSEX_FLAG;
                }
                else
                {
                    DEBUGPRINT_MIDI2_InqFlags &= (uint8)~DEBUGPRINT_INQ_SYSEX_FLAG;
                }
            }
            else if(cmd == DEBUGPRINT_SYSEX_ENDS_WITH1)
            {
            ...
            {
                /* Identify Request support */
                if ((DEBUGPRINT_MIDI2_InqFlags & DEBUGPRINT_INQ_SYSEX_FLAG) != 0u)
                {
                    DEBUGPRINT_MIDI2_InqFlags &= (uint8)~DEBUGPRINT_INQ_SYSEX_FLAG;
                    ...
                    {   /* Set the flag about received the Identity Request.
                        *  The Identity Reply message may be send by user code.
                        */
                        DEBUGPRINT_MIDI2_InqFlags |= DEBUGPRINT_INQ_IDENTITY_REQ_FLAG;
                    }
                }
            }
            else /* Do nothing for other command */
            ...
            if ((cmd >= DEBUGPRINT_NOTE_OFF) && ( cmd <= DEBUGPRINT_PITCH_BEND_CHANGE))
            {
                if (DEBUGPRINT_MIDI2_TxRunStat == epBuf[DEBUGPRINT_EVENT_BYTE1])
                {   /* Skip the repeated Status byte */
                    i++;
                }
                else
                {   /* Save Status byte for next event */
                    DEBUGPRINT_MIDI2_TxRunStat = epBuf[DEBUGPRINT_EVENT_BYTE1];
                }
            }
            else
            {   /* Clear Running Status */
    ...
    #ifdef DEBUGPRINT_MIDI2_PROCESS_USB_OUT_EXIT_CALLBACK
        DEBUGPRINT_MIDI2_ProcessUsbOut_ExitCallback();
    #endif /* (DEBUGPRINT_MIDI2_PROCESS_USB_OUT_EXIT_CALLBACK) */
    }
#endif /* (DEBUGPRINT_MIDI_EXT_MODE >= DEBUGPRINT_TWO_EXT_INTRF) */
#endif /* (DEBUGPRINT_MIDI_EXT_MODE >= DEBUGPRINT_ONE_EXT_INTRF) */
...
#endif  /* defined(DEBUGPRINT_ENABLE_MIDI_STREAMING) */
