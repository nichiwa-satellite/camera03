...
volatile uint8 DEBUGPRINT_hidProtocol[DEBUGPRINT_MAX_INTERFACES_NUMBER];
 ...
 * reload timer.*/
volatile uint8 DEBUGPRINT_hidIdleRate[DEBUGPRINT_MAX_INTERFACES_NUMBER];

/** This variable contains the timer counter, which is decremented and reloaded 
 * by the USBFS_UpdateHIDTimer() API.*/
volatile uint8 DEBUGPRINT_hidIdleTimer[DEBUGPRINT_MAX_INTERFACES_NUMBER]; /* HID device idle rate value */
...
/*******************************************************************************
* Function Name: DEBUGPRINT_UpdateHIDTimer
****************************************************************************//**
*
...
*
*******************************************************************************/
uint8 DEBUGPRINT_UpdateHIDTimer(uint8 interface) 
{
    uint8 stat = DEBUGPRINT_IDLE_TIMER_INDEFINITE;

    if(DEBUGPRINT_hidIdleRate[interface] != 0u)
    {
        if(DEBUGPRINT_hidIdleTimer[interface] > 0u)
        {
            DEBUGPRINT_hidIdleTimer[interface]--;
            stat = DEBUGPRINT_IDLE_TIMER_RUNNING;
        }
        else
        {
            DEBUGPRINT_hidIdleTimer[interface] = DEBUGPRINT_hidIdleRate[interface];
            stat = DEBUGPRINT_IDLE_TIMER_EXPIRED;
        }
    }
...
/*******************************************************************************
* Function Name: DEBUGPRINT_GetProtocol
****************************************************************************//**
*
...
*
*******************************************************************************/
uint8 DEBUGPRINT_GetProtocol(uint8 interface) 
{
    return(DEBUGPRINT_hidProtocol[interface]);
}


/*******************************************************************************
* Function Name: DEBUGPRINT_DispatchHIDClassRqst
****************************************************************************//**
*
...
uint8 DEBUGPRINT_DispatchHIDClassRqst(void) 
{
    ...
    uint8 interfaceNumber = (uint8) DEBUGPRINT_wIndexLoReg;
    ...
    if (0u != (DEBUGPRINT_bmRequestTypeReg & DEBUGPRINT_RQST_DIR_D2H))
    {
        ...
        switch (DEBUGPRINT_bRequestReg)
        {
            case DEBUGPRINT_GET_DESCRIPTOR:
                if (DEBUGPRINT_wValueHiReg == DEBUGPRINT_DESCR_HID_CLASS)
                {
                    DEBUGPRINT_FindHidClassDecriptor();
                    if (DEBUGPRINT_currentTD.count != 0u)
                    {
                        requestHandled = DEBUGPRINT_InitControlRead();
                    }
                }
                else if (DEBUGPRINT_wValueHiReg == DEBUGPRINT_DESCR_HID_REPORT)
                {
                    DEBUGPRINT_FindReportDescriptor();
                    if (DEBUGPRINT_currentTD.count != 0u)
                    {
                        requestHandled = DEBUGPRINT_InitControlRead();
                    }
            ...
            case DEBUGPRINT_HID_GET_REPORT:
                DEBUGPRINT_FindReport();
                if (DEBUGPRINT_currentTD.count != 0u)
                {
                    requestHandled = DEBUGPRINT_InitControlRead();
                }
            ...
            case DEBUGPRINT_HID_GET_IDLE:
                ...
                if ((interfaceNumber < DEBUGPRINT_MAX_INTERFACES_NUMBER) && (DEBUGPRINT_wValueLoReg == 0u)) 
                {
                    DEBUGPRINT_currentTD.count = 1u;
                    DEBUGPRINT_currentTD.pData = &DEBUGPRINT_hidIdleRate[interfaceNumber];
                    requestHandled  = DEBUGPRINT_InitControlRead();
                }
                break;
                
            case DEBUGPRINT_HID_GET_PROTOCOL:
                /* Validate interfaceNumber */
                if( interfaceNumber < DEBUGPRINT_MAX_INTERFACES_NUMBER)
                {
                    DEBUGPRINT_currentTD.count = 1u;
                    DEBUGPRINT_currentTD.pData = &DEBUGPRINT_hidProtocol[interfaceNumber];
                    requestHandled  = DEBUGPRINT_InitControlRead();
                }
                break;
    ...
    {   
        ...
        switch (DEBUGPRINT_bRequestReg)
        {
            case DEBUGPRINT_HID_SET_REPORT:
                DEBUGPRINT_FindReport();
                if (DEBUGPRINT_currentTD.count != 0u)
                {
            ...
            case DEBUGPRINT_HID_SET_IDLE:
                /* This function does not support multiple reports per interface */
                /* Validate interfaceNumber and Report ID (should be 0): Do not support Idle per Report ID */
                if ((interfaceNumber < DEBUGPRINT_MAX_INTERFACES_NUMBER) && (DEBUGPRINT_wValueLoReg == 0u))
                {
                    DEBUGPRINT_hidIdleRate[interfaceNumber] = (uint8)DEBUGPRINT_wValueHiReg;
                    ...
                    */
                    if(DEBUGPRINT_hidIdleRate[interfaceNumber] <
                       DEBUGPRINT_hidIdleTimer[interfaceNumber])
                    {
                        /* Set the timer to zero and let the UpdateHIDTimer() API return IDLE_TIMER_EXPIRED status*/
                        DEBUGPRINT_hidIdleTimer[interfaceNumber] = 0u;
                    }
                    ...
                    */
                    else if(DEBUGPRINT_hidIdleTimer[interfaceNumber] <= 1u)
                    {
                        /* Do nothing.
                    ...
                    {   /* Reload the timer*/
                        DEBUGPRINT_hidIdleTimer[interfaceNumber] =
                        DEBUGPRINT_hidIdleRate[interfaceNumber];
                    }
                    requestHandled = DEBUGPRINT_InitNoDataControlTransfer();
                }
                break;

            case DEBUGPRINT_HID_SET_PROTOCOL:
                /* Validate interfaceNumber and protocol (must be 0 or 1) */
                if ((interfaceNumber < DEBUGPRINT_MAX_INTERFACES_NUMBER) && (DEBUGPRINT_wValueLoReg <= 1u))
                {
                    DEBUGPRINT_hidProtocol[interfaceNumber] = (uint8)DEBUGPRINT_wValueLoReg;
                    requestHandled = DEBUGPRINT_InitNoDataControlTransfer();
                }
                break;
...
void DEBUGPRINT_FindHidClassDecriptor(void) 
{
    ...
    interfaceN = (uint8) DEBUGPRINT_wIndexLoReg;
    ...
    pTmp = &pTmp[DEBUGPRINT_interfaceSetting[interfaceN]];
    ...
    DEBUGPRINT_currentTD.count = *pDescr;
    DEBUGPRINT_currentTD.pData = pDescr;
}
...
void DEBUGPRINT_FindReportDescriptor(void) 
{
    ...
    DEBUGPRINT_currentTD.count =  ((uint16)((uint16) pDescr[1u] << 8u) | pDescr[0u]);
    DEBUGPRINT_currentTD.pData = &pDescr[2u];
}


/*******************************************************************************
* Function Name: DEBUGPRINT_FindReport
****************************************************************************//**
*
...
void DEBUGPRINT_FindReport(void) 
{
...
#ifdef DEBUGPRINT_FIND_REPORT_CALLBACK
    DEBUGPRINT_FindReport_Callback();
#endif /* (DEBUGPRINT_FIND_REPORT_CALLBACK) */
    
    DEBUGPRINT_currentTD.count = 0u;   /* Init not supported condition */
    pTmp = DEBUGPRINT_GetConfigTablePtr(DEBUGPRINT_configuration - 1u);
    reportType = (uint8) DEBUGPRINT_wValueHiReg;
    interfaceN = (uint8) DEBUGPRINT_wIndexLoReg;
    ...
    /* USB_DEVICEx_CONFIGURATIONy_INTERFACEz_TABLE */
    pTmp = (const T_DEBUGPRINT_LUT CYCODE *) pTmp->p_list;
    if (interfaceN < DEBUGPRINT_MAX_INTERFACES_NUMBER)
    {
        /* Now use Alternate setting number */
        pTmp = &pTmp[DEBUGPRINT_interfaceSetting[interfaceN]];
        ...
        if ((reportType >= DEBUGPRINT_HID_GET_REPORT_INPUT) &&
            (reportType <= DEBUGPRINT_HID_GET_REPORT_FEATURE))
        {
            /* Get the entry proper TD (IN, OUT or Feature Report Table)*/
            pTmp = &pTmp[reportType - 1u];
            
            /* Get reportID */
            reportType = (uint8) DEBUGPRINT_wValueLoReg;
            
            /* Validate table support by the HID descriptor, compare table count with reportID */
            if (pTmp->c >= reportType)
            {
                pTD = (T_DEBUGPRINT_TD *) pTmp->p_list;
                pTD = &pTD[reportType];                          /* select entry depend on report ID*/
                DEBUGPRINT_currentTD.pData = pTD->pData;   /* Buffer pointer */
                DEBUGPRINT_currentTD.count = pTD->count;   /* Buffer Size */
                DEBUGPRINT_currentTD.pStatusBlock = pTD->pStatusBlock;
            }
        }
    }
}
